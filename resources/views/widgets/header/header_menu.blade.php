            ...
            <div id="sp-menu" class="span10">
                <div id="sp-main-menu" class="visible-desktop">
                    <ul class="sp-menu level-0">
                        ...
                        <li class="menu-item {{ check_menu($menu_1->id,'parent')}}">
                            <a href="/{{ $menu_1->slug }}.html" class="menu-item {{ check_menu($menu_1->id)}}">
                                <span class="menu">
                                    <span class="menu-title">{{ $menu_1->name }}</span>
                                </span>
                            </a>
                            @if(check_menu($menu_1->id,'parent'))
                                                                    ...
                                                                    <div class="cmenu_jshopping_block">
                                                                        <div id="vina-cmenu-vertical-jshopping201" class="vina-cmenu-vertical-jshopping">
                                                                            <ul class="level0">
                                                                                ...
                                                                                <li class="menu-item {{ check_menu($menu_2->id,'has_sub')}}">
                                                                                    <a href="{{ check_type_menu($menu_1->slug) }}/{{ $menu_2->slug }}.html" title="{{ $menu_2->name }}"> <span class="catTitle ">{{ $menu_2->name }}</span> </a>
                                                                                    @if(check_menu($menu_2->id))
                                                                                    <ul class="sub-menu"><?php
                                                                                        $menu3 = \App\Menu::where('parent_id',$menu_2->id)->get();
                                                                                        ?>
                                                                                        @foreach($menu3 as $menu_3)
                                                                                        <li class="menu-item">
                                                                                            <a href="{{ check_type_menu($menu_1->slug) }}/{{ $menu_3->slug }}.html" title="{{ $menu_3->name }}"> <span class="catTitle ">{{ $menu_3->name }}</span> </a>
                                                                                        </li>
                                                                                        @endforeach
                                                                                    </ul>
                                                                                    @endif
                        ...
                        <div class="mod-content clearfix">
                            <div class="mod-inner clearfix">
                                <div class="custom custom_call_us">
                                    <div class="tell-phone"><span style="color: #ffffff;"><strong>HOTLINE: <a style="color: #ffffff;" href="tel:{{ page_get_option('hotline') }}">{{ page_get_option('hotline') }}</a></strong></span>
                                    </div>
                                </div>
                            </div>
                        </div>
